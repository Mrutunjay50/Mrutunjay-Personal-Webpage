...
const makeLinkSmooth = ()=>{
    const navLinks = document.querySelectorAll(".navbar_links1");
    // console.log(navLinks);

    for(let n in navLinks){
        if(navLinks.hasOwnProperty(n)){
            // console.log(navLinks[n])
...
/*const scrollActive = ()=>{
    ...
    window.onscroll = ()=>{
        ...
        for(let n in sections){
            if(sections.hasOwnProperty(n) && sections[n].offsetTop <= scrollPosition){
                const id = sections[n].id;
                console.log(id);
                document.querySelector(".navbar_links1").classList.add("active")
            }
        }
    }
}
